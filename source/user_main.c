...
//			}
	...
	*OUT2_ADDR = _OUT2_DATA; //迅速输出
	...
	if(M_ChkFlag(SL_IN1_MIANK)!=0 && M_ChkFlag(SL_SERIESTOPING)==0 && M_ChkFlag(SL_SERIESTOP)==0 && M_ChkFlag(SL_ERRSTOPING)==0 && M_ChkFlag(SL_ERRSTOP)==0 && M_ChkFlag(SL_SYN_S3)!=0)//M_ChkFlag(SL_MPR_SYNOK)!=0)
	{
...
//			if(M_ChkFlag(SL_HV_STATE)==0 && (TRS_NPR_U.ampflt>(1.10* NGS_Udq_p_ex)))//2013-12-6小于0.91延时1ms置位
			if(M_ChkFlag(SL_HV_STATE)==0 && (_MPR_IQ_Kd == 1000))
			{
				gridfault_flag_flt4++;
		...
		if((M_ChkFlag(SL_HV_STATE)!=0))		//恢复正常态的判断
		{		
			if(M_ChkCounter(MAIN_LOOP.cnt_hv_rcv,500)>0)			//50ms 加大判断时间，防止因为网侧前（40ms）发无功拉低电压意外跳出，电压恢复也需50ms才能退出
			{
	...
	if(*(BANK_RAMEND + 1) != 0x55AA)
	{
		...
		if(M_ChkFlag(SL_RAMBANKSAVE)==0)  																//PC指令未要求数据锁存，处于刷新状态
		{
			...
			else if(_NPR_ID_Kd==1000)
			{
				...
				*(BANK_RAMSTART+((Uint32)RAM_BIAS * 17 + BANK_RAMDATA_POS)) = (int16)(PHAI_d*1000);					//17=定子磁链d轴分量
				*(BANK_RAMSTART+((Uint32)RAM_BIAS * 18 + BANK_RAMDATA_POS)) = (int16)(PHAI_q*1000);				    //18=定子磁链q轴分量
